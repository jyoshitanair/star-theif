...
func _ready() -> void:
	for i in range(0,5):
		...
		temp_card.position = positions_array[i]
		add_child(temp_card)
		set(card_name, temp_card)
		set(card_name_end, (temp_card.position.y -198))
...
func _process(delta: float) -> void:
	if first1 ||first2 ||first3 ||first4 ||first5:
		for i in range(0,5):
			...
			if actual_card1:
				var target_pos = lerp(actual_card1.position.y,get(card_name_end1), delta*10 )
